...
'Call InitGitRepo(GitProjectFolderName)    ' Initialize Git Repository if not already existing.
...
Private Sub InitGitRepo(ByVal GitProjectFolderName As String)
...
If FileOrDirExists(GitProjectFolderName & ".git" & "\") And _
   FileOrDirExists(GitProjectFolderName & ".git\info\") And _
   FileOrDirExists(GitProjectFolderName & ".git\objects\") And _
   FileOrDirExists(GitProjectFolderName & ".git\refs\") And _
   FileOrDirExists(GitProjectFolderName & ".git\config") And _
   FileOrDirExists(GitProjectFolderName & ".git\description") And _
   FileOrDirExists(GitProjectFolderName & ".git\hooks\") And _
   FileOrDirExists(GitProjectFolderName & ".git\HEAD") Then GoTo Exit_Sub

XX = "A new Git Repo being initialized for: " & Application.CurrentProject.Name & vbCrLf & vbCrLf
ZZ = Replace(GitHubRemoteAdd, "REPOSITORY", GitRepoName)

If GitHubRemoteAdd = "" Then
  Response = MsgBox(XX & "GitHubRemoteAdd = """" so no Remote Repo will be attached. " & vbCrLf & vbCrLf & _
                    "Continue?  No will abort the run so that ""GitHubRemoteAdd"" variable can be set properly.", vbYesNo)
 Else
  Response = MsgBox(XX & "GitHubRemoteAdd = """ & ZZ & """ will be used to set the Remote Repo. " & vbCrLf & vbCrLf & _
                    "Continue?  No will abort the run so that ""GitHubRemoteAdd"" variable can be set properly.", vbYesNo)
End If
...
Call MakeAndRunBatchFile(XX, GitProjectFolderName)
...
GitPushText = _
   ...
   "echo ." & vbCrLf & _
   "set /p TryAgain=Are you ready to try to ""git push"" again? (Y or N) " & vbCrLf
